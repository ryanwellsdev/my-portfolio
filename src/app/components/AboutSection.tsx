...
const AboutSection: React.FC<AboutSectionProps> = ({ isOpen, onClick }) => {
  ...
  return (
    <div className="absolute left-0 top-0 h-full flex items-center">
      ...
      >
        <div
          className={`h-full flex flex-col p-8 transition-opacity duration-500 ease-in-out ${
      ...
      {/* Toggle Button */}
      <div
        className="h-full w-[6rem] py-6 text-nowrap text-sm flex flex-col justify-between items-center border-l-2 border-r-2 bg-[#f8f8f8] border-black z-10"
        onClick={onClick}
        role="button"
        tabIndex={0}
        onKeyDown={(event) => {
      ...
      >
        ...
        {/* Adjusted margin-top */}
        <span className="text-black rotate-90 mb-7">© Ryan Wells</span>{" "}
